...
// Main Idea; the medai has fixed number of numbers before it.

// ***** Questions: 
//! 1. first hwo it is making sure that the number is existing in the vector means it is caluclating 
// just a mid without making sure wheter the never is existing or not ?

// * Ans: Got it actually if you see the mechanism cleary so you will find that it is trying to get the lowest number whose count is greater than the  number
//* of count (elements before the median without including it) and also searching for the highest number whose number of elements before and including it
//* is just greater than count. --> so by this basically we are finding the lowest possible answer whose number of elements before and including it is equal 
//* to the count which is nothing but the median itself and that's how we are making sure that the assumed median is existing. 

//! 2. why we have to give lo as to increment the count? while calculating the (totla number  of numbers less than the given number in the entire array)\
